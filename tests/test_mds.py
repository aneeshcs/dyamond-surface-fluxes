...
def test_open_2d_variable_values_and_time(fake_mit):
    ...
    # Sign convention attrs attached for the flux machinery: per mit/readme.txt
    # this dataset stores fluxes positive-UP, so to_positive_down must flip.
    from dyamond_fluxes.fluxes import to_positive_down

    assert "upward" in da.attrs["long_name"]
